...
import { Eye, ShieldAlert, Radio, Search, Filter } from 'lucide-react';
...
function AdminConversationsContent() {
  ...
  const handleSelect = (conv: Conversation) => {
    ...
    router.replace(`/admin/conversations?convId=${conv.id}`);
  };

  const filteredConversations = conversations.filter((c) => {
    if (selectedTeamFilter === 'ALL') return true;
    return c.team_id === selectedTeamFilter;
  });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold">Team Conversation Oversight</h1>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-950 text-amber-300 border border-amber-800">
                LIVE AUDIT
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              Authorized real-time monitoring of 1-on-1 private team chats under your administration.
            </p>
          </div>
        </div>
...
export default function AdminConversationsPage() {
  return (
    <DashboardLayout allowedRoles={['ORGANIZATION_ADMIN', 'PLATFORM_SUPER_ADMIN']}>
      <Suspense fallback={<div className="p-8 text-center text-xs text-slate-400">Loading audit feed...</div>}>
